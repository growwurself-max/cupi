...
interface Screen1ChapterProps {
  config: ExperienceConfig
  onBegin: () => void
}

export function Screen1Chapter({ config, onBegin }: Screen1ChapterProps) {
  return (
    <ScreenShell className="surface-obsidian">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_35%,rgba(252,217,166,0.14),transparent_55%)]"
      />
      ...
      >
        <motion.div
          aria-hidden
          animate={{ scale: [1, 1.26, 1], opacity: [0.3, 0.7, 0.3] }}
          transition={{ duration: 3.6, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute -inset-6 rounded-full bg-amber-400/15 blur-2xl"
        />
        <span className="relative flex h-32 w-32 items-center justify-center rounded-full border border-amber-200/40 bg-white/10 text-6xl backdrop-blur-md">
      ...
      >
        <motion.button
          type="button"
          onClick={onBegin}
          animate={{ boxShadow: ['0 0 24px -6px rgba(252,217,166,0.4)', '0 0 52px -6px rgba(245,184,97,0.8)', '0 0 24px -6px rgba(252,217,166,0.4)'] }}
