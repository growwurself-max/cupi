...
export function TeaserScreen({ config, onBegin }: TeaserProps) {
  return (
    <ScreenShell>
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,rgba(143,123,255,0.16),transparent_55%)]"
      />
      ...
      >
        <span className="glass-panel rounded-full px-4 py-1.5 text-xs font-bold tracking-[0.2em] uppercase"
      ...
      >
        <button
          type="button"
          onClick={onBegin}
          className="group flex min-h-14 items-center gap-2.5 rounded-full bg-gradient-to-r from-[#ff9bb3] to-[#8f7bff] px-8 text-base font-bold text-obsidian-900 transition-transform duration-200 hover:scale-[1.05] active:scale-95"
        >
